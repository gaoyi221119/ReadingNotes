...
首先，它能够明确指定类型，有类型检查功能。 其次，可以使用C++的作用域规则将定义限制在特定的函数或文件中。 第三，可以将const用于更复杂的类型，比如数组和结构。
...
逗号运算符 到目前为止，逗号运算符最常见的用途是将两个或更多的表达式放到一个for循环表达式中。逗号运算符的特性有下面几个：
...
有用的字符函数库cctype 从C语言继承而来，老式格式是ctype.h，常用的有： 1.png

快排中中值的选取： 将元素每5个一组，分别取中值。在n/5个中值里面找到中值，作为partition的pivot。 为什么*不每3个一组？保证pivot左边右边至少3n/10个元素，这样最差O(n)。

C++存储方案：C++三种，C++11四种 这些方案的区别就在于数据保留在内存中的时间。
...
自己写string类注意事项：
关于记录已有对象数object_count 不要在类声明（即头文件）中初始化静态成员变量，这是因为声明描述了如何分配内存，但并不分配内存。对于静态类成员，可以在类声明之外使用单独的语句来进行初始化，这是因为静态类成员是单独存储的，而不是对象组成部分。请注意，初始化语句指出了类型int（不可缺少），并使用了作用域运算符，但没有使用关键字static。 初始化是在方法文件中，而不是在类声明文件中进行的，这是因为类声明位于头文件中，可能被包含多次，这样若在头文件中进行初始化静态成员，将出现多个初始化语句副本，从而引发错误。 对于不能在类声明中初始化静态成员的一种例外情况是：静态数据成员为整型或枚举型const。即如果静态数据成员是整型或枚举型，则可以在类声明中初始化。
...
代码首先检查自我复制，这是通过查看赋值操作符右边的地址(&s)是否与接收对象(this)的地址相同来完成的，如果相同，程序将返回*this,然后结束。 如果不同，释放str指向的内存，这是因为稍后将把一个新字符串的地址赋给str。如果不首先使用delete操作符，则上述字符串将保留在内存中。由于程序程序不再包含指向字符串的指针，一次这些内存被浪费掉。 接下来的操作与复制构造函数相似，即为新字符串分配足够的内存空间，然后复制字符串。 赋值操作并不创建新的对象，因此不需要调整静态数据成员num_strings的值。

重载运算符最好声明为友元 比如将比较函数作为友元，有助于将String对象与常规的C字符串进行比较。例如，假设answer是String对象，则下面的代码： if("love" == answer) 将被转换为： if(operator == ("love", answer)) 然后，编译器将使用某个构造函数将代码转换为： if(operator == (String("love"), answer)) 这与原型是相匹配的。

在重写string类时使用中括号访问字符时注意： （1）为什么重载的[]返回值是个char &而不是char？ （2）为什么有两个重载[]的版本，另一个是const版本？

解答（1）： 将返回类制声明为char &,便可以给特定元素陚值。例如，可以编写这样的代码： String means ("might"); means [9] = ' r'; 第二条语句将被转换为一个重载运算符函数调用： means.operator[][0] = 'r'; 这里将r陚给方法的返回值，而函数返回的是指向means.str[0]的引用，因此上述代码等同于下面的代码： means.str[0] = 'r'； 代码的最后一行访问的是私有数据，但由于operator 是类的一个方法，因此能够修改数组的内容。 最终的结果是“might”被改为“right”。

解答（2）： 假设有下面的常量对象： const String answer("futile")； 如果只有上述operator定义，则下面的代码将出错： cout << answer[1]; // compile-time error 原因是answer是常量，而上述方法无法确保不修改数据（实际上，有时该方法的工作就是修改数据， 因此无法确保不修改数据)。 但在重载时，C++将区分常量和非常量函数的特征标，因此可以提供另一个仅供const String对象使用 的 operator版本： // for use with const String objects const char & string::operator[](int i) const { return str[i]; } 有了上述定义后，就可以读/写常规String对象了 ：而对于const Siring对象，则只能读取其数据。
...
其次，继承会引起很多问题，尤其从多个基类继承时，可能必须处理很多问题，如包含同名方法的独立的基类或共亨祖先的独立基类。 总之，使用包含不太可能遇到这样的麻烦。
另外，包含能够包括多个同类的子对象。如果某个类需要3个string对象，可以使用包含声明3个独立的string成员。而继承则只能使用一个这样的对象（当对象都没有名称时，将难以区分）。
然而，私有继承所提供的特性确实比包含多。例如，假设类包含保护成员（可以是数据成员，也可以是成员函数），则这样的成员在派生类中足可用的，但在继承层次结构外是不可用的。如果使用组合将这样的类包含在另一个类中，则后者将不是派生类，而是位于继承层次结构之外，因此不能访问保护成员。但通过继承得到的将是派生类，因此它能够访问保护成员。 另—种需要使用私有继承的情况是需要重新定义虛函数。派生类可以重新定义虚函数，但包含类不能。使用私有继承，重新定义的函数将只能在类中使用，而不是公有的。

通常，应使用包含来建立has-a关系；如果新类需要访问原有类的保护成员，或需要重新定义 虚函數，则应使用私有继承。

关于保护继承 保护继承是私有继承的变体，保护继承在列出基类时使用关键字protected;
class Student : protected std::string,
                    protected std::valarray<double>
{
...
}
使用保护继承时，基类的公有成员和保护成员都将成为派生类的保护成员，和私有继承一样，基类的接口在派生类中也是可用的，但在继承层次结构之外是不可用的。当从派生类派生出另一个类的时，私有继承和保护继承 之间的主要区别便呈现出来了。使用私有继承时，第三代将不能使用基类的接口，这是因为基类的共有方法在派生类中将变成私有方法；使用保护继承时，基类的公有方法在第二代中将编程呢个受保护的，因此第三代派生类可以使用它们。

下表总结了公有、私有和保护继承。隐式向上转换意味着无需进行显式类型转换，就可以将基类指针或引用指向派生类对象。 2.png

智能指针相关 请参考：C++智能指针简单剖析，推荐必看。
C++中的容器种类：
序列容器（7个）
...
priority_queue：是另一个适配器类，支持的操作与queue相同。 priority_queue模板类是另一个适配器类，它支持的操作与queue相同。两者之间的主要区别在于，在priority_queue中，最大的元素被移到对首。内部区别在于，默认的底层类是vector。可以修改用于确定哪个元素放到队首的比较方式，方法是提供一个可选的构造函数参数：
